...
public class QueryPlan {
    ...
    public Iterator<Record> execute() {
        ...
        Map<Set, QueryOperator> newmap = new HashMap<>();
		...
		tablename.add(this.startTableName);
		map.put(tablename, this.minCostSingleAccess(this.startTableName));

		for (int i = 0; i < this.joinTableNames.size(); i++) {
            Set<String> tablename1 = new HashSet<>();
    		tablename1.add(joinTableNames.get(i));
    		map.put(tablename1, this.minCostSingleAccess(joinTableNames.get(i)));
    	}
    	...
    	newmap = map;
    	for (int i = 0; i < this.joinTableNames.size(); i++) {
    		newmap = this.minCostJoins(newmap, map);
    	}
        ...
        this.finalOperator = this.minCostOperator(newmap);
    	this.addGroupBy();
        this.addProjects();
    ...
    QueryOperator minCostSingleAccess(String table) {
        ...
        for (int i : this.getEligibleIndexColumns(table)) {
        	minOp = new IndexScanOperator(transaction, table, this.selectColumnNames.get(i), this.selectOperators.get(i), this.selectDataBoxes.get(i));
        	if (minOp.cost <= minCost) {
        		minIndex = i;
        		minCost = minOp.cost;
        	}
        }
        ...
        return this.addEligibleSelections(minOp, minIndex);
    }
    ...
    Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
        ...
        //get the left side and the right side (table name and column)
        		...
        		String leftColName = getJoinLeftColumnNameByIndex(i)[1];
        		String rightTableName = getJoinRightColumnNameByIndex(i)[0];
        		String rightColName = getJoinRightColumnNameByIndex(i)[1];
        /*
         ...
         */
        		...
        		QueryOperator currMin;
        		String addName;
        		if (settable.contains(leftTableName) && !settable.contains(rightTableName)) {
        			addName = rightTableName;
            		Set<String> rightTableSet = new HashSet<>();
            		rightTableSet.add(rightTableName);
		        	currMin = this.minCostJoinType(prevMap.get(settable), pass1Map.get(rightTableSet),
		        			leftColName, rightColName);
        		} else if (!settable.contains(leftTableName) && settable.contains(rightTableName)) {
        			addName = leftTableName;
            		Set<String> leftTableSet = new HashSet<>();
            		leftTableSet.add(leftTableName);
		        	currMin = this.minCostJoinType(prevMap.get(settable), pass1Map.get(leftTableSet),
        		...
        		currentset.add(addName);
        		int minCost = map.containsKey(currentset) ? map.get(currentset).estimateIOCost() : -1;
        		if (minCost > currMin.estimateIOCost()) {
        			map.replace(currentset, currMin);
        		} else if (minCost == -1){
        			map.put(currentset, currMin);
        		}
        	}
