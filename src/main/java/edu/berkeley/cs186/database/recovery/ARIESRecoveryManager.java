...
public class ARIESRecoveryManager implements RecoveryManager {
    ...
    public long commit(long transNum) {
        ...
        transactionTable.get(transNum).lastLSN = record.getLSN();
    ...
    public long end(long transNum) {
        // TODO(hw5): implement
    	if (transactionTable.get(transNum).transaction.getStatus().equals(Status.ABORTING)) {
    		reverse(transNum, -1L);
    	}
    ...
    public long logPageWrite(long transNum, long pageNum, short pageOffset, byte[] before,
        ...
        if (before.length <= BufferManager.EFFECTIVE_PAGE_SIZE / 2) {
        	UpdatePageLogRecord upRecord = new UpdatePageLogRecord(transNum, pageNum, tte.lastLSN, pageOffset, before, after);
            logManager.appendToLog(upRecord);
        	tte.lastLSN = upRecord.getLSN();
        	dirtyPageTable.putIfAbsent(pageNum, upRecord.getLSN());
            tte.touchedPages.add(pageNum);
            return upRecord.getLSN();
        } else {
        	UpdatePageLogRecord unRecord = new UpdatePageLogRecord(transNum, pageNum, tte.lastLSN, pageOffset, before, null);
        	UpdatePageLogRecord reRecord = new UpdatePageLogRecord(transNum, pageNum, tte.lastLSN, pageOffset, null, after);
            logManager.appendToLog(unRecord);
            logManager.appendToLog(reRecord);
        	tte.lastLSN = reRecord.getLSN();
        	dirtyPageTable.putIfAbsent(pageNum, unRecord.getLSN());
            tte.touchedPages.add(pageNum);
            return reRecord.getLSN();
        }
    }
    ...
    public void checkpoint() {
        ...
        for (Entry<Long, Long> entry : dirtyPageTable.entrySet()) {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
        	...
        	txnTable.put(entry.getKey(), new Pair<>(tte.transaction.getStatus(), tte.lastLSN));
        }
    ...
    public Runnable restart() {
    	...
    	for (Map.Entry<Long, Long> entry: dirtyPageTable.entrySet()) {
    		if (bufferManager.fetchPage(getPageLockContext(entry.getKey()).parentContext(), entry.getKey(), false).getPageLSN() >= logManager.getFlushedLSN()) {
    ...
    void restartAnalysis() {
        ...
        while (iter.hasNext()) {
        	LogRecord curlr = iter.next();
        	
        	if (checkWithoutFlushPageOperation(curlr) | checkWithFlushPageOperation(curlr) | checkPartOperation(curlr)) {
	    		Long curTransNum = curlr.getTransNum().get();

        		if (!transactionTable.containsKey(curTransNum)) {
        			transactionTable.put(curTransNum, new TransactionTableEntry(newTransaction.apply(curTransNum)));
        		}
        		TransactionTableEntry tte = transactionTable.get(curTransNum);
        		tte.lastLSN = curlr.LSN;
        		if (aboutPage(curlr)) {
            		Long curPageNum = curlr.getPageNum().get();
            		tte.touchedPages.add(curPageNum);
            		this.acquireTransactionLock(tte.transaction, this.getPageLockContext(curPageNum), LockType.X);
        			if (checkWithoutFlushPageOperation(curlr)) {
	        			dirtyPageTable.putIfAbsent(curPageNum, curlr.getLSN());
	        		} else if (checkWithFlushPageOperation(curlr)) {
	        			logManager.flushToLSN(curlr.getLSN());
        	...
        	if (checkStatus(curlr)) {
        		...
        		TransactionTableEntry tte = transactionTable.get(curTransNum);
        		tte.lastLSN = curlr.LSN;
        		if (curlr.getPageNum().isPresent()) {
            		Long curPageNum = curlr.getPageNum().get();
            		tte.touchedPages.add(curPageNum);
            		this.acquireTransactionLock(tte.transaction, this.getPageLockContext(curPageNum), LockType.X);
        		}
    			if (curlr.type.equals(LogType.END_TRANSACTION)) {
        			tte.transaction.cleanup();
        			transactionTable.remove(curTransNum);
        			tte.transaction.setStatus(Status.COMPLETE);
        		} else if (curlr.type.equals(LogType.ABORT_TRANSACTION)) {
        			tte.transaction.setStatus(Status.RECOVERY_ABORTING);
        		} else if (curlr.type.equals(LogType.COMMIT_TRANSACTION)) {
        			tte.transaction.setStatus(Status.COMMITTING);
        		}
        	}
        	
        	// BeginCheckpoint record
        	if (curlr.type.equals(LogType.BEGIN_CHECKPOINT)) {
        		updateTransactionCounter.accept(curlr.getMaxTransactionNum().get());
        	}
	        
        	// EndCheckpoint record
        	if (curlr.type.equals(LogType.END_CHECKPOINT)) {
                for (Map.Entry<Long, Long> entry: curlr.getDirtyPageTable().entrySet()) {
//                	dirtyPageTable.putIfAbsent(entry.getKey(), entry.getValue());
                	dirtyPageTable.replace(entry.getKey(), entry.getValue());
                }
                for (Map.Entry<Long, Pair<Status, Long>> entry: curlr.getTransactionTable().entrySet()) {
                	if (curlr.getTransNum().isPresent()) {
    	    		Long curTransNum = curlr.getTransNum().get();
                	if (!transactionTable.containsKey(curTransNum)) {
                		transactionTable.put(entry.getKey(), new TransactionTableEntry(newTransaction.apply(curTransNum)));
                	}
            		TransactionTableEntry tte = transactionTable.get(curTransNum);
            		if (tte.lastLSN < entry.getValue().getSecond()) {
            			transactionTable.get(entry.getKey()).lastLSN = entry.getValue().getSecond();
            		}
                	if (!entry.getValue().getFirst().equals(Status.COMPLETE)) {
                		for (Long curPageNum: curlr.getTransactionTouchedPages().get(curTransNum)) {
                			tte.touchedPages.add(curPageNum);
                    		this.acquireTransactionLock(tte.transaction, this.getPageLockContext(curPageNum), LockType.X);                			
                		}
                	} else {
    	...
    	for (Entry<Long, TransactionTableEntry> entry: transactionTable.entrySet()) {
    		TransactionTableEntry tte = entry.getValue();
    		if (tte.transaction.getStatus().equals(Status.COMMITTING)) {
    			tte.transaction.cleanup();
    			tte.transaction.setStatus(Status.COMPLETE);
                logManager.appendToLog(new EndTransactionLogRecord(entry.getKey(), tte.lastLSN));
                transactionTable.remove(entry.getKey());
    		} else if (tte.transaction.getStatus().equals(Status.RUNNING)) {
    			tte.transaction.setStatus(Status.RECOVERY_ABORTING);
    			LogRecord tmplr = new AbortTransactionLogRecord(entry.getKey(), tte.lastLSN);
    ...
    void restartRedo() {
        // TODO(hw5): implement
    	Long lrecLSN = Long.MAX_VALUE;
        for (Entry<Long, Long> entry : dirtyPageTable.entrySet()) {
        	if (entry.getValue() < lrecLSN) {
        		lrecLSN = entry.getValue();
        	}
        }
        if (lrecLSN == Long.MAX_VALUE) {
        	return;
        }
        Iterator<LogRecord> iter = logManager.scanFrom(lrecLSN);
        while (iter.hasNext()) {
        	LogRecord curlr = iter.next();
        	if (curlr.isRedoable()) {
        		if (aboutPage(curlr)) {
        			...
        			if (dirtyPageTable.containsKey(curPageNum)) {
        				if (curlr.getLSN() >= dirtyPageTable.get(curPageNum)) {
        					Page curPage = bufferManager.fetchPage(getPageLockContext(curPageNum).parentContext(), curPageNum, false);
        					if (curPage.getPageLSN() < curlr.getLSN()) {
        	        			curlr.redo(diskSpaceManager, bufferManager);
        					}
        				}
        			}
        		} else if (aboutPart(curlr)) {
        			curlr.redo(diskSpaceManager, bufferManager);
        		}
        	}
        }
    ...
    void restartUndo() {
        ...
        for (Map.Entry<Long, TransactionTableEntry> entry : transactionTable.entrySet()) {
        	if (!entry.getValue().transaction.getStatus().equals(Status.RECOVERY_ABORTING)) {
        		continue;
        	}
        ...
        while (pq.size() > 0) {
        	...
        	if (curlr.isUndoable()) {				
        		...
        		LogRecord tmplr = curlr.undo(tte.lastLSN).getFirst();
        		logManager.appendToLog(tmplr);
        		if (curlr.undo(tte.lastLSN).getSecond()) {
    ...
    // Helpers ///////////////////////////////////////////////////////////////////////////////
    public void reverse(long transNum, long LSN) {
    	LogRecord cr = logManager.fetchLogRecord(transactionTable.get(transNum).lastLSN);
		while(true) {
			if (cr.getLSN() <= LSN) {
				break;
			}
			if (cr.isUndoable()) {
				LogRecord tmplr = cr.undo(cr.getLSN()).getFirst();
				update(cr, tmplr);
			}
			if (cr.getPrevLSN().isPresent()) {
				cr = logManager.fetchLogRecord(cr.getPrevLSN().get());
			} else {
				break;
			}
		}
    ...
    public void update(LogRecord cr, LogRecord lr) {
    	logManager.appendToLog(lr);
		if (cr.undo(cr.getLSN()).getSecond()) {
			logManager.flushToLSN(cr.getLSN());
		}
		lr.redo(diskSpaceManager, bufferManager);
    }
    
    boolean aboutPart(LogRecord lr) {
