...
public class Histogram {
    ...
    public void buildHistogram(Table table, int attribute) {
    	...
    	Record tmpr;
    	float tmp;
    	BacktrackingIterator<Record> tableiter = table.iterator();
    	Record rmin = tableiter.next();
    	Record rmax = rmin;
    	while (tableiter.hasNext()) {
    		tmpr = tableiter.next();
    		tmp = this.quantization(tmpr, attribute);
    		if (tmp < this.quantization(rmin, attribute)) {
    			rmin = tmpr;
    		} else if (tmp > this.quantization(rmin, attribute)) {
    			rmax = tmpr;
    		}
    	}
    	this.minValue = this.quantization(rmin, attribute);
    	this.maxValue = this.quantization(rmax, attribute);
    	...
    	while (tableiter.hasNext()) {
    		tmp = this.quantization(tableiter.next(), attribute);
    		buckets[this.bucketIndex(tmp)].increment(tmp);
    	}
    ...
    private float [] allGreaterThan(float qvalue) {
        ...
        if (qvalue >= buckets[buckets.length - 1].getEnd()) {
            for (int i = 0; i < buckets.length; i++) {
            	result[i] = 0f;
            }
            return result;
        }
        if (qvalue < buckets[0].getStart()) {
            for (int i = 0; i < buckets.length; i++) {
            	result[i] = 1f;
            }
            return result;
        ...
        while (count < buckets.length) {
        	if (buckets[count].getEnd() <= qvalue) {
        		result[count] = 0f;
        	} else if ((buckets[count].getStart() <= qvalue) && (buckets[count].getEnd() > qvalue)) {
        		result[count] = (buckets[count].getEnd() - qvalue) / (float) this.width;
        	} else if (buckets[count].getStart() > qvalue) {
        		result[count] = 1f;
        	}
        	count ++;
        }
    ...
    private float [] allLessThan(float qvalue) {
        ...
        while (count < buckets.length) {
        	if (buckets[count].getEnd() <= qvalue) {
        		...
        		result[count] = (qvalue - buckets[count].getStart()) / (float) this.width;
        	} else if (buckets[count].getStart() > qvalue) {
        		result[count] = 0f;
        	}
        	count ++;
