...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
    	...
    	if (tmp.isPresent()) {
    		List<DataBox> rootKey = new ArrayList<DataBox>();
        	rootKey.add(tmp.get().getFirst());
        	List<Long> rootChild = new ArrayList<Long>();
        	rootChild.add(root.getPage().getPageNum());
        	rootChild.add(tmp.get().getSecond());
    		InnerNode newRoot = new InnerNode(metadata, bufferManager, rootKey, rootChild, lockContext);
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
    	...
    	public BPlusTreeIterator(LeafNode leaf, DataBox key) {
    		this.leaf = leaf;
    		this.iter = leaf.scanGreaterEqual(key);
    	}
