...
public class TestLockManager {
    ...
    private ResourceName tableResource;
...
//        System.out.println("1");
    ...
    public void testRelease() {
        ...
        runner.run(0, () -> {
            lockman.acquire(transactions[0], tables[0], LockType.IS);
        });
        runner.run(0,  () -> {
            lockman.acquire(transactions[0], tables[1], LockType.S);
        });
        runner.run(0,  () -> {
            lockman.release(transactions[0], tables[1]);
        });
        assertEquals(LockType.IS, lockman.getLockType(transactions[0], tables[0]));
        runner.joinAll();
    }
...
//
//        ResourceName r0 = dbResource;
//        ResourceName r1 = tableResource;
//
//        runner.run(0, () -> lockman.acquire(t1, r0, LockType.IS));
//        runner.run(1, () -> lockman.acquire(t2, r0, LockType.IX));
//
//        assertTrue(holds(lockman, t1, r0, LockType.IS));
//        assertTrue(holds(lockman, t2, r0, LockType.IX));
//
//        runner.run(0, () -> lockman.acquire(t1, r1, LockType.S));
//        runner.run(1, () -> lockman.acquire(t2, r1, LockType.X));
